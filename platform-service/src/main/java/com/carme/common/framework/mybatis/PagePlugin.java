...
public class PagePlugin implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (page == null) {
            ...
            Object realParam = getParam(parameter);
            invocation.getArgs()[1] = realParam;
            return handlePaging(invocation, realParam, page);
        }
    ...
    protected Object handlePaging(Invocation invocation, Object parameter, Page page)
                                                                                     throws Exception {
        ...
        // 查询结果集
        StaticSqlSource sqlsource = new StaticSqlSource(configuration, getLimitString(
            boundSql.getSql(), page), boundSql.getParameterMappings());
        MappedStatement.Builder builder = new MappedStatement.Builder(configuration,
            "id_temp_result", sqlsource, SqlCommandType.SELECT);
        ...
        //设置到page对象
        page.setResultList(data);
        page.setRecordCount(getTotalSize(invocation, configuration, mappedStatement, boundSql,
            parameter));
     ...
     * @throws Exception
     */
    protected Object exeQuery(Invocation invocation, MappedStatement query_statement)
                                                                                     throws Exception {
     ...
     */
    @SuppressWarnings("rawtypes")
    protected int getTotalSize(Invocation invocation, Configuration configuration,
                               MappedStatement mappedStatement, BoundSql boundSql, Object parameter)
                                                                                                    throws Exception {
        ...
        if (map_statement.containsKey(count_id)) {
            // 优先查找能统计条数的sql
            List data = (List) exeQuery(invocation, mappedStatement.getConfiguration()
                .getMappedStatement(count_id));
            if (data.size() > 0) {
            ...
            totalSize = getTotalSize(configuration, mappedStatement, boundSql, countSql,
     ...
     * @return
     */
    protected int getTotalSize(Configuration configuration, MappedStatement mappedStatement,
                               BoundSql boundSql, String countSql, Connection connection,
                               Object parameter) throws SQLException {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        int totalSize = 0;
        try {
            ParameterHandler handler = configuration.newParameterHandler(mappedStatement,
                parameter, boundSql);
    ...
    public Object getParam(Object parameter) {
